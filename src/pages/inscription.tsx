...
export default function InscriptionPage() {
  ...
  axios
    .post('http://localhost:3000/auth/register',  { email, password, nom, prenom, status })
    .then((response) => {
...
const items = [
  ...
  return (
    <body>
      <div className="iAll">
        <div className="zoom">
          <form onSubmit = {handleRegistration} action="" method="post" className="iForm">
            <h1 className="titre">Inscription</h1>
            <div className="name">
              <div className="ChampNom">
                <input type="" placeholder="Nom" onChange={(e) => setNom(e.target.value)} required/>
                <MdOutlineGesture className="iIconne" />
              </div>
              <div className="ChampNom">
                <input type="" placeholder="Prénom" onChange={(e) => setPrenom(e.target.value)} required />
                <MdOutlineGesture className="iIconne" />
              </div>
            </div>
            ...
            <div className="iChampRenseignemants">
              <input type="email" placeholder="Adresse email" onChange={(e) => setEmail(e.target.value)} pattern='[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$' required/>
